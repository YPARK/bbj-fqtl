...
queue_fgwas: $(foreach k, 50, jobs/20180612/fgwas-$(k).txt.gz)

queue_fgwas_long: $(foreach k, 50, jobs/20180612/fgwas-$(k)-long.gz)

jobs/20180612/fgwas-%.txt.gz:
	[ -d $(dir $@) ] || mkdir -p $(dir $@)
	seq 1 $(nLD) | awk '{ print "./make.fgwas.R" FS $$1 FS "$(LD)" FS "$(DATA)" FS "$(TRAITS_STR)" FS ("result/20180606/confounder/$*/" $$1 ".txt.gz") FS $* FS ("result/20180612/fgwas/$*/" $$1) FS "FALSE" }' | gzip > $@
...
jobs/20180612/fgwas-%-long.gz: jobs/20180612/fgwas-%.txt.gz
	@[ -d $(dir $@) ] || mkdir -p $(dir $@)
...
################################################################
queue_summary: $(foreach k, 50, jobs/20180612/summary-$(k).txt.gz)

jobs/20180612/summary-%.txt.gz:
	@[ -d $(dir $@) ] || mkdir -p $(dir $@)
	echo "./make.combine-fgwas.R result/20180612/fgwas/$*/ $(LD) 0.5 result/20180612/fgwas-$*-05.txt.gz" | gzip > $@
	echo "./make.combine-fgwas.R result/20180612/fgwas/$*/ $(LD) 0.9 result/20180612/fgwas-$*-09.txt.gz" | gzip >> $@
