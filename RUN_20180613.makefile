...
jobs/20180613/fgwas-%.txt.gz:
	[ -d $(dir $@) ] || mkdir -p $(dir $@)
	seq 1 $(nLD) | awk '{ print "./make.fgwas.R" FS $$1 FS "$(LD)" FS "$(DATA)" FS "$(TRAITS_STR)" FS ("result/20180606/confounder/$*/" $$1 ".txt.gz") FS $* FS ("$(RDIR)/fgwas/$*/" $$1) FS "FALSE" }' | gzip > $@
	seq 1 $(nLD) | awk '{ print "./make.fgwas.R" FS $$1 FS "$(LD)" FS "$(DATA)" FS "$(TRAITS_STR)" FS ("result/20180606/confounder/$*/" $$1 ".txt.gz") FS $* FS ("$(RDIR)/fgwas_nn/$*/" $$1) FS "TRUE" }' | gzip >> $@
...
################################################################
cluster: $(foreach pip, 5 9, $(foreach k, 50, $(RDIR)/cluster/fgwas_nn-$(k)-0$(pip).trait.txt.gz $(RDIR)/cluster/fgwas-$(k)-0$(pip).trait.txt.gz))

$(RDIR)/cluster/fgwas%.trait.txt.gz: $(RDIR)/fgwas%.txt.gz
	@[ -d $(dir $@) ] || mkdir -p $(dir $@)
	Rscript --vanilla ./make.clustering.R $< 200 $(shell echo $@ | sed 's/.trait.txt.gz//g')
